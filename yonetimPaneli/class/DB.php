...
class DB
{
  ...
  `date` date NOT NULL
    ...
    {
        // Dosya adından uzantıyı al ve küçük harfe çevir
        $parca = explode(".", $dosyaadi);
        $uzanti = strtolower(end($parca));
        return $uzanti;
    }
    ...
    public function upload($nesnename, $yuklenecekyer = 'images/', $tur = 'img', $w = '', $h = '', $resimyazisi = '')
    {
        ...
        if (!empty($_FILES[$nesnename]["name"])) {
            ...
            // Resim türü kontrolü
            if ($tur == "img" && in_array($uzanti, ['png', 'jpg', 'jpeg', 'gif'])) {
                $classIMG = new upload($_FILES[$nesnename]);

                // Yükleme başarılı mı?
                if ($classIMG->uploaded) {
                    // Resim yeniden boyutlandırma işlemleri
                    if (!empty($w)) {
                        if (!empty($h)) {
                            // Genişlik ve yükseklik verildiğinde
                            $classIMG->image_resize = true;
                            $classIMG->image_x = $w;
                            $classIMG->image_y = $h;
                        } else {
                            // Sadece genişlik verildiğinde, yüksekliği orantılı ayarla
                        ...
                        // Sadece yükseklik verildiğinde, genişliği orantılı ayarla
                    ...
                    // Resim üzerine yazı ekleme işlemi
                    if (!empty($resimyazisi)) {
                        $classIMG->image_text = $resimyazisi;
                        $classIMG->image_text_direction = 'v';
                        $classIMG->image_text_color = '#FFFFFF'; // Beyaz yazı
                        $classIMG->image_text_position = 'BL'; // Alt sol
                    }
                    ...
                    if ($classIMG->processed) {
                        ...
                        return "Hata: " . $classIMG->error; // Hata mesajını döndür
                    }
                } else {
                    return "Hata: Dosya yüklenemedi"; // Yükleme başarısız
                }
            } elseif ($tur == "ds" && in_array($uzanti, ['doc', 'docx', 'pdf', 'xlsx', 'xls', 'ppt', 'xml', 'mp4', 'avi', 'mov'])) {
                // Diğer dosya türleri için yükleme
                $classIMG = new upload($_FILES[$nesnename]);
                if ($classIMG->uploaded) {
                    $rand = uniqid(true);
                    $classIMG->file_new_name_body = $rand;
                    ...
                    if ($classIMG->processed) {
                        ...
                        return "Hata: " . $classIMG->error;
                    }
                } else {
                    return "Hata: Dosya yüklenemedi";
                }
            } else {
                return "Hata: Geçersiz dosya türü";
            }
        } else {
